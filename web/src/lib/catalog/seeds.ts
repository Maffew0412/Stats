/**
 * Canonical seed data for chains, departments, and generic concepts.
 *
 * Single source of truth used by both:
 * - scripts/seed.ts to populate the database
 * - the client-side List Builder UI for offline autocomplete
 *
 * When the catalog grows beyond what's reasonable to ship to the browser,
 * the UI can switch to fetching from the database via an API route. For now
 * the data is small (kilobytes) and shipping it statically keeps the UI
 * snappy and DB-free.
 */

export const CHAIN_SEEDS = [
...
export type DepartmentSlug = (typeof DEPARTMENT_SEEDS)[number]['slug'];

export interface GenericConceptSeed {
  slug: string;
  name: string;
...
export const GENERIC_CONCEPT_SEEDS: GenericConceptSeed[] = [
  // Dairy
